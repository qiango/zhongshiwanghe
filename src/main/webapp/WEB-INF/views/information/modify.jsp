   ...
   <form action="" target="_self" method="post" id="submit_form"
       ...
       <div class="col-xs-11 stretch form-group float-none">
            ...
            <div class="col-xs-10 stretch">
               <select name="competition_id" class="drop-btn width-200 select-choose form-control" data-placeholder="请选择">
               		<option value=""></option>
                     <c:forEach items="${competition_list}" var="competition">
                      <c:choose>
                            <c:when test="${information.competition_id == competition.competition_id  }">
                        <option value="${competition.competition_id }" selected="selected">${competition.competition_name }</option>
                            </c:when>    
                            <c:otherwise>
       ...
       <div class="col-xs-11 stretch form-group float-none">
         ...
         <div class="col-xs-11 stretch form-group float-none">
            ...
            <div class="col-xs-10 stretch">
            	<input type="text" class="form-control width-350 display-align" name="information_title"  value="" required
                placeholder="输入资讯标题" >
            </div>
        </div>
        <div class="col-xs-11 stretch form-group float-none">
            ...
            <div class="col-xs-10 stretch">
            	<textarea class="form-control width-350 display-align" name="information_abstract" rows="5"
    ...
    cadleo_data('/information/modifyLoad.htmls?information_id='+sessionStorage.getItem('edit_id'));
    function showData(data){
    	...
    	.done(function(){
        	$('select option').each(function(){
    	    	data.data.info_query[$(this).parent().attr('name')]==$(this).val() && $(this).prop('selected',true);
    	    });
    	})
    	.done(function(){
	    	...
	    	$(':input').not('select,input[type="checkbox"]').each(function(){
	    		$(this).val(data.data.info_query[$(this).attr('name')]);
	    	});
	    	$('.viewImg').attr('src','pic.htmls?p=' + (data.data.info_query.image_list[0]?data.data.info_query.image_list[0].information_image_url:''));
	    	$('#submit_file').val((data.data.info_query.image_list[0]?data.data.info_query.image_list[0].information_image_url:''));
    	});
    	
	    var editor = new UE.ui.Editor();
	    editor.render("editor");
    	UE.getEditor('editor').addListener("ready", function () {
    	   UE.getEditor('editor').setContent(data.data.info_query.information_content, false);
    	});
    }	
    $("#submit_form").validate({
        submitHandler : function(){
            if(confirm("确定要提交数据吗？")) {
                $.ajax({
                     type: "POST",
                     url: url + "/information/modifySave.htmls",
