...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
	...
	<div class="order-title stretch">
		<div class="title-text col-xs-3 padding-collapse">俱乐部管理</div>
	</div>
	...
	function showData(data){
		...
		for(var i = 0; i < data.length; i++ ){
			trs += '<tr>\
					...
					<td>'+(data[i].club_applicant_name||'')+'</td>\
					<td>'+(data[i].name||'')+'</td>\
					<td>'+(data[i].language_name||'')+'</td>\
					<td>'+(data[i].club_create_date||'')+'</td>\
					<td>'+(data[i].club_status||'')+'</td>\
					<td class="">\
						<a href="#/club/modify" edit_id="'+data[i].club_id+'" class="fa fa-pencil-square-o v-align wj_edit_data" title="编辑"></a>\
						<a href="javascript:void(0);" delete_id="'+data[i].club_id+'" delete_path="/club/delete.htmls?club_id='+ data[i].club_id +'" back_path="/club/index" class="fa fa-trash-o wj_delete_data" title="删除"></a>\
						<a href="javascript:void(0);" cog_path="/club/sportscamp" club_id="'+data[i].club_id+'" edit_id="'+data[i].club_id+'" class="fa fa-cog wj_cog_data" title="配置"></a>\
					</td>\
				  </tr>';
		}
