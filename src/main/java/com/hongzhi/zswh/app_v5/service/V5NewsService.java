...
import com.hongzhi.zswh.app_v5.entity.V5VideoEntity;
 ...
 */
@Service
public class V5NewsService {
    
    @Autowired
    private V5NewsDao v5NewsDao;
    
    @Autowired
    private V5iQiYiService iqiyi;
     ...
     * @return
     */
    public List<V5LatestNewsEntity> latestNews(String competition_id, String club_id, Integer page_number,String news_type) {
...
//            head=NEWS.VIDEO_HEAD;
        ...
        List<V5LatestNewsEntity> news_list = v5NewsDao.latestNewsList(news_type_list,range_type,competition_id_list,club_id_list,startRow,NEWS.PAGE_SIZE,2);
    ...
    public Object  newsList(SessionProperty properties,Integer page_number, String news_type){
        ...
        List<V5LatestNewsEntity> news_list =  latestNews(competition_id, club_id, page_number, news_type);
        List<Integer> news_id_list = new ArrayList<>();
        for(int i=0;i<news_list.size();i++){
            news_id_list.add(news_list.get(i).getNews_id());
        }
        List<V5NewsImageEntity> image_list = new ArrayList<>();
        if(news_id_list.size()>0){
            image_list = v5NewsDao.newsImageByNewsIdList(news_id_list);
        }
        String head = NEWS.PIC_HEAD ;
//        if(news_type.toLowerCase().equals("latest")){
//            head=NEWS.PIC_HEAD;
...
//        }
        ...
        if(news_type.toLowerCase().equals("latest")){
            ...
            List<V5VideoEntity> video_list = new ArrayList<>();
            for(int i=0;i<news_list.size();i++){
                V5VideoEntity videoEntity = new V5VideoEntity();
    ...
    /**   Twitter : @taylorwang789 
     * Creat time : Jun 3, 2016    10:26:45 AM
     * @param news_id
     * @param model
     * @return
     */
    public String newsDetail(String news_id, Model model) {
        V5NewsDetailEntity news_detail = v5NewsDao.selectNewsById(Integer.parseInt(news_id));
        List<V5NewsImageEntity> image_list = v5NewsDao.newsImageByNewsIdList(Arrays.asList(Integer.parseInt(news_id)));
        for(int i=0;i<image_list.size();i++){
            image_list.get(i).setMedia_url(image_list.get(i).getMedia_url().replace("HEAD", NEWS.PIC_HEAD));
        }
        news_detail.setImage_list( image_list );
        model.addAttribute("information", news_detail );
        if(news_detail.getNews_type().equals(NEWS.TYPE_DOCUMENT)){
            return "/news/appInformationDetail";
        }else if( news_detail.getNews_type().equals(NEWS.TYPE_IMAGES)){
            return "/news/appInformationImages";
        }else{
            model.addAttribute("video", iqiyi.getVideo(news_id) );
            model.addAttribute("recommend", newsListRecommend(null,1 , Integer.valueOf(news_id)) );
            return "/news/share";
        }
    }
    ...
    public  String[]  getUserCompetitionAndClub(Integer user_id){
		...
		List<String> userCompetition = v5NewsDao.getUserCompetition(user_id);
		String competion_id="";
		...
		String club_id="";
    	List<String> userClub = v5NewsDao.getUserClub(user_id);
    ...
    public Object newsListRecommend(SessionProperty properties, Integer page_number, Integer news_id) {
        ...
        List<V5VideoEntity> video_list = (List<V5VideoEntity>) map.get("news");
